...
const Home = () => {
  ...
  useEffect(() => {
    fetch('https://road-safety-backend-862776753006.asia-south1.run.app/api/config/fetch-states')
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    if (filters.state) {
      fetch(`https://road-safety-backend-862776753006.asia-south1.run.app/api/config/fetch-cities?state=${filters.state}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    <div className="w-screen h-screen grid grid-cols-1 lg:grid-cols-[2fr_3fr] overflow-hidden">
      ...
      <div className="flex flex-col justify-center items-center lg:items-start p-10 bg-gray-100">
        ...
        <Card className="w-full max-w-lg shadow-lg bg-white p-6 rounded-lg mt-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Select onValueChange={(value) => setFilters({ ...filters, state: value, city: "" })}>
              ...
              <Select onValueChange={(value) => setFilters({ ...filters, city: value })} disabled={!filters.state}>
                <SelectTrigger className="w-full p-3 border rounded-md">
            ...
            <Button variant="outline" className="w-full p-3 rounded-md" onClick={handleSearch}>
              Search Reports
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="mt-6">
          <Button variant="outline" className="p-3 rounded-md" onClick={() => navigate("/NewReport")}>
            Report a New Issue
          </Button>
        </div>
      </div>
      ...
      <div className="w-full h-full">
        {location.lat && location.lon ? (
          <MapContainer center={[location.lat, location.lon]} zoom={13} className="w-full h-full">
            <TileLayer
          ...
          <p className="flex items-center justify-center h-full text-gray-600">Fetching location...</p>
        )}
      </div>
